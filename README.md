...
### Using the run2.sh Script with Persistent Storage

The `run2.sh` script creates persistent storage mapped to local directories:

```bash
# Make script executable if needed
chmod +x run2.sh

# Run the container with local persistent storage
./run2.sh
```

This script:
- Creates local directories (`deploy/`, `data/`, `examples/`) for bind mounts
- Pre-populates directories from the image on first run if empty
- Fixes permissions for proper file access

The container uses bind mounts to local directories:
- `./deploy` - Karaf deploy directory for OSGi bundles
- `./data` - Karaf runtime data, configuration, and bundle cache
- `./examples` - Example Camel routes
...
### Using Podman Volumes (run.sh)

Data and logs are stored in named volumes:
- `karaf_data` - Karaf runtime data, deployments, and configuration
- `karaf_logs` - Application and system logs

### Using Bind Mounts (run2.sh)

Persistent storage mapped to local directories:
- `./deploy` - Karaf deploy directory for OSGi bundles
- `./data` - Karaf runtime data, configuration, and bundle cache
- `./examples` - Example Camel routes

The `run2.sh` script automatically initializes these directories from the image on first run and fixes permissions for proper access.
...
- Create examples for karaf camel xml dsl and java also 
